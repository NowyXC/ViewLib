package com.nowy.viewlib.views.titlebar;
...
public class TitleBar extends Toolbar {
    ...
    private void setupData(final Context context, AttributeSet attrs) {
        ...
        boolean leftVisibility = a.getBoolean(R.styleable.TitleBar_tbar_leftVisibility,true);
        if(!leftVisibility){
            hideLeftImg();
        }
    ...
    private void init(Context context) {
        mTitleBarHeight = getResources().getDimensionPixelSize(R.dimen.tbar_height);
        setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,mTitleBarHeight));
        setBackgroundResource(R.color.tbar_bg_titleBar);
        mBarView =inflate(context, R.layout.title_bar,this);
        mContentView =  mBarView.findViewById(R.id.title_bar_contentView);
...
//            layoutParams.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
    ...
    public static class Builder {
        ...
        public TitleBar attach(@IdRes int layoutIds) {
            ...
            ViewGroup rootView =  ((Activity) P.mContext).findViewById(layoutIds);
            return attach(rootView);
        }

        /**
         * 传入当前根布局，将titleBar绑定到rootView中
         * @param rootView
         * @return
         */
        public TitleBar attach(ViewGroup rootView) {
            TitleBar titleBar = create();
            if (rootView instanceof LinearLayout) {
                rootView.addView(titleBar, 0);
            } else if(rootView instanceof ConstraintLayout){//约束布局的 MATCH_PARENT 并不能达到填充满父布局的效果
                ConstraintLayout.LayoutParams layoutParams = new ConstraintLayout.LayoutParams(
                        ConstraintLayout.LayoutParams.MATCH_CONSTRAINT_PERCENT,
                        titleBar.getTitleBarHeight() //指定高度为配置高度
                );
                layoutParams.width = 0;
                layoutParams.matchConstraintPercentWidth = 1; //100%
