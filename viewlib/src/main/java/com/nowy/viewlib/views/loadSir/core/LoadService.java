package com.nowy.viewlib.views.loadSir.core;
...
import com.nowy.viewlib.views.loadSir.callback.SuccessCallback;
    ...
    public void showSuccessDelayed(long delay) {
        if(delay > 0){
            loadLayout.postDelayed(new Runnable() {
                @Override
                public void run() {
                    Activity activity = ViewUtil.getActivity(loadLayout);
